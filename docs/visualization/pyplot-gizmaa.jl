...
From gizmaa's gist: https://gist.github.com/gizmaa/7214002
===#
...
ax.arrow(
    x[convert(Int64,floor(length(x)/2))],
    0.4,
    0.0009,
    0.4,
    head_width=0.001,
...
ax.arrow(
    x[convert(Int64,floor(0.3length(x)))]-0.25dx,
...
## Julia dictionary objects are automatically converted to Python object when they pass into a PythonPlot function
...
majorformatter = mpl.dates.DateFormatter("%d.%m.%Y")
minorformatter = mpl.dates.DateFormatter("%H:%M")
majorlocator = mpl.dates.DayLocator(interval=1)
minorlocator = mpl.dates.HourLocator(byhour=(8, 16))
